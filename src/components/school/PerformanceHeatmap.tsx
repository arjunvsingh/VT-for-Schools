                ...
                <div className="grid grid-cols-6 gap-2">
                    ...
                    {DATA.flatMap((row, rIdx) =>
                        row.scores.map((cell, cIdx) => {
                            ...
                            if (cell.value < 70) colorClass = "bg-red-500";
                            else if (cell.value < 85) colorClass = "bg-yellow-500";
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={cn(
                                        "rounded-md relative group cursor-pointer h-10 lg:h-12",
                                        colorClass,
                                        // Opacity based on intensity (simulated here with just 3 buckets for simplicity, 
                                        // could use rgba for finer grain)
                                        "opacity-80 hover:opacity-100 hover:scale-105 transition-all"
                                    )}
                                >
                                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 text-xs font-bold text-stone-black">
                                        {cell.value}
                                    </div>
                                </motion.div>
                            );
